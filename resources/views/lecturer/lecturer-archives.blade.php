<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Archived Lecturers') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-row justify-between">
                <form action="{{route('lecturer')}}" method="GET">
                  ...
                  <tbody>
                    @if ($lecturers->count())
                        ...
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-center">{{$lecturer->deleted_at}}</td>
                            <td class="py-2 px-4 border-b text-center">
                                <div class="inline-block">
                                    <form action="{{route('lecturer.restore',$lecturer->id)}}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button>
                                ...
                                <div class="inline-block">
                                    <form action="{{route('lecturer.forcedelete',$lecturer->id)}}" method="POST" onsubmit="return confirm('Are You Sure to Remove this Data?');">
                                        @csrf
                                        @method('DELETE')
                                        <button>
