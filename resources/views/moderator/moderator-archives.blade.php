<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Archived Moderators') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex flex-row justify-between">
                <form action="{{route('moderator')}}" method="GET">
                ...
                <table class="min-w-full bg-white border border-gray-300 border-separate">
                  <thead>
                    <tr>
                        ...
                        <th class="py-2 px-4 border-b bg-gray-300">Name</th>
                        <th class="py-2 px-4 border-b bg-gray-300">Email</th>
                        <th class="py-2 px-4 border-b bg-gray-300">Phone Number</th>
                        <th class="py-2 px-4 border-b bg-gray-300">Address</th>
                  ...
                  <tbody>
                    @if ($moderators->count())
                        ...
                        <tr>
                            ...
                            <td class="py-2 px-4 border-b text-center">{{$moderator->deleted_at}}</td>
                            <td class="py-2 px-4 border-b text-center">
                                <div class="inline-block">
                                    <form action="{{route('moderator.restore',$moderator->id)}}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button>
                                ...
                                <div class="inline-block">
                                    <form action="{{route('moderator.forcedelete',$moderator->id)}}" method="POST" onsubmit="return confirm('Are You Sure to Remove this Data?');">
                                        @csrf
                                        @method('DELETE')
                                        <button>
